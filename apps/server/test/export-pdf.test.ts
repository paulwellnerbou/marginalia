...
describe('PDF export', () => {
  ...
  chromiumTest(
    'GET /:uid/export.pdf returns a PDF with the expected headers',
    async () => {
  ...
  chromiumTest(
    'GET /:uid/export.pdf filename derives from the document title when name is unset',
    async () => {
  ...
  chromiumTest(
    'GET /:uid/export.pdf falls back to uid when no title is derivable',
    async () => {
      const created = await upload(CLIENT_A, {
  ...
  chromiumTest(
    'GET /:uid/export.pdf respects ?theme= and produces different bytes per theme',
    async () => {
  ...
  chromiumTest(
    'GET /:uid/export.pdf embeds attached image assets as data URLs',
    async () => {
      const created = await upload(CLIENT_A, {
        markdown: '# Doc\n\n![logo](logo.png)\n',
        name: 'With logo',
  ...
  chromiumTest(
    'GET /:uid/export.pdf renders a mermaid diagram to SVG on the page',
    async () => {
  ...
  chromiumTest(
    'GET /:uid/export.pdf?mermaid=mmdr pre-rasterizes diagrams without the mermaid runtime',
    async () => {
  ...
  chromiumTest(
    'GET /:uid/export.pdf rejects path-traversal-shaped theme names cleanly',
    async () => {
  ...
  chromiumTest(
    'GET /:uid/export.pdf blocks outbound requests to disallowed hosts (SSRF)',
    async () => {
  ...
  chromiumTest(
    'firewall blocks http:// even to an allowlisted host',
    async () => {
  ...
  chromiumTest(
    'inlineImageAssets targets the src attribute even when alt text shares the ref name',
    async () => {
  ...
  chromiumTest(
    'mermaid-wait timeout is reported as 504 export-timeout, not 500',
    async () => {
