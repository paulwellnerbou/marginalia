...
describe('renderMermaidWithChromium', () => {
  ...
  test.if(SHOULD_RUN_CHROMIUM_TESTS)(
    'produces an SVG with diagram content',
    async () => {
  ...
  test.if(SHOULD_RUN_CHROMIUM_TESTS)(
    'PNG resolution scales with pngScale; natural dims stay fixed',
    async () => {
  ...
  test.if(SHOULD_RUN_CHROMIUM_TESTS)(
    'returns null on parse failure (graceful degrade)',
    async () => {
      ...
      const result = await renderMermaidWithChromium('not a diagram, just words', 'svg');
