...
rejected instead, the comment stays open for someone to resolve. Reopening a proposal puts a
decision back in the card, so a comment an accept resolved reopens with it; one a person
resolved stays resolved. When the feedback is *"almost — change one thing"*, don't open a
second proposal: `update_proposal` swaps in new text while the thread, its links and the
discussion stay put.
