...
function instructions(config: McpConfig): string {
  return [
    ...
    'document seen before, or one listed under `folder:` in a document header: documents of a',
    'folder share their links, so reading one with its link opens the rest by uid.',
    '',
    'Working through a review:',
    '  1. list_threads (state="open") — read the reviewer’s comments, each with the text it',
    '     is anchored to. `needs_proposal: true` narrows it to the comments no proposal',
    '     answers yet, which is the backlog for "turn these comments into edit proposals".',
    ...
    '     text comes from.',
    '  3. create_proposal — suggest the concrete rewrite, with `answers_thread_ids` listing',
    '     every comment it answers. That records a real link: each reviewer sees',
    '     "See proposed change" on their comment, and accepting the proposal resolves them',
    '     — except one another open proposal also answers. One rewrite of a paragraph can',
    '     settle every comment anchored in it.',
